...
{
	document.bbsForm.hiBbsSeq.value = bbsSeq; //bbsseq를 빼고 임의의 값 대입시 오류 발생 
	document.bbsForm.action="/board/view";    //2를 만들고 싶은 사람은 이쪽 위치에 board/view2를 만들면됨.
	document.bbsForm.submit();
}
		...
		<c:choose>
			...
			<td>
				...
				<c:if test="${hiBoard.hiBbsIndent >0 }">
						<img src="/resources/images/icon_reply.gif" style="margine-left:${hiBoard.hiBbsIndent}em" />
				</c:if>
...
<c:if test="${!empty paging}">
	<c:if test="${paging.prevBlockPage gt 0}"> <!-- 이전 블럭에 대한 구문 gt->0보다 큰 것을 의미함 -->
	...
	<c:forEach var="i" begin="${paging.startPage}" end="${paging.endPage}">
		<c:choose>
			...
			<c:otherwise> <!-- otherwise= 현재 페이지(결론) -->
